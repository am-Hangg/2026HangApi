...
const getAllProducts = async (query) => {
  ...
  const limit = query.limit ?? 10; // for page pagination
...
const createProduct = async (data,files, userId) => {
  ...
  return await Product.create({
    ...data,
    imagesUrls: uploadedFiles.map((file) => file.url),
    createdBy: userId
  });
};
...
const updateProduct = async (id, input, files) => {
  ...
  if (files && files.length>0) {
    ...
    updateData.imagesUrls = uploadedFiles.map((file) => file.url);
  }
...
const getTotalCount = async () => {
  return await Product.countDocuments(); // for total product count
};
